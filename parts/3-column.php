<div class="three-column-container">
	<div class="fw-container">
		<div class="three-column">
			<div class="column">
				<img src="<?php echo get_template_directory_uri (); ?>/images/startup.jpg">
				<div class="column-text">
					<h2>Startups</h2>
					<h4>Applications for the Virginia Velocity Tour are now closed</h4>
					<p>Check back soon to learn which startups were selected as finalists for the tour.</p>
			</div>
			</div>
			<div class="column">
				<img src="<?php echo get_template_directory_uri (); ?>/images/investor.jpg">
				<div class="column-text">
					<h2>Investors</h2>
					<h4>Connect with early-stage founders from across the Commonwealth</h4>
					<p>Email <a href="mailto:investors@virginiavelocitytour.org" target="_blank">investors@virginiavelocity.org</a> if you’d like to be involved.</p>
				</div>
			</div>
			<div class="column">
				<img src="<?php echo get_template_directory_uri (); ?>/images/image1.jpg">
				<div class="column-text">
					<h2>Community</h2>
					<h4>Schedule and details about the tour are now available.</h4>
					<p><a href="http://virginiavelocitytour.org/schedule">View the full schedule.</a></p>
				</div>
			</div>
		</div>
	</div>
</div>